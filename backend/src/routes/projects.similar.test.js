...
describe("GET /api/projects/:id/similar", () => {
  ...
  test("returns similar projects by category using SIMILARITY ordering", async () => {
    ...
    // SIMILARITY must be ordered by the source project's name
    const similarSql = pool.query.mock.calls[1][0];
    expect(similarSql).toContain("SIMILARITY(name, $3)");
    expect(similarSql).toContain("category = $1");
    expect(similarSql).toContain("id != $2");
    expect(similarSql).toContain("status = 'active'");
    expect(pool.query.mock.calls[1][1]).toEqual(["Reforestation", "proj-1", "Amazon Reforestation"]);
  });
