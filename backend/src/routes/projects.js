 ...
 *
 * Returns decoded on-chain donation events for a project, fetched from the
 * Stellar Horizon server and decoded via the project's contract.
 *
 * @route GET /api/projects/:id/on-chain-donations
 * @param {import('express').Request} req - Express request containing the project id.
 * @param {import('express').Response} res - Express response object.
 * @param {import('express').NextFunction} next - Express error middleware.
 * @returns {Promise<void>} Sends the paginated on-chain donation events.
 * @throws {Error} If the database lookup or event decoding fails.
 */
