...
fuzz_target!(|data: &[u8]| {
    ...
    // Cap the amount to i64::MAX to prevent unrecoverable Soroban VM aborts.
    // Values beyond this are unrealistic donation amounts and cause internal
    // arithmetic overflows that abort() rather than panic(), making them
    // uncatchable by catch_unwind.
    let amount = raw_amount.min(i64::MAX as u64);
    ...
    // Execute donate() call with fuzzed input parameters using try_donate()
    // which gracefully captures business logic and host errors as Result.
    let _ = client.try_donate(
        &token,
        &donor,
        &target_proj_id,
        &(amount as i128),
